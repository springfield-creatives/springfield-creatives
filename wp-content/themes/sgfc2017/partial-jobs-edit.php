...
if(empty($entry_id)){
	...
	sgfc_get_editable_posts($current_user->ID, 'jobs', 'Job Posting');
	...
	if(!sgfc_has_editor_permission($entry_id, $current_user->ID)){
		...
		<form method="post">
			<input type="hidden" name="sgfc_job_profile_edit" value="1">
			<input type="hidden" name="sgfc_job_profile_id" value="<?php echo get_the_ID() ?>">
			...
			acf_form(array(
				'field_groups' => array(SGFC_JOB_ACF_ID), // the ID of the field group
				'post_id' => $entry_id,
				'form' => false
			));
