...
if(empty($entry_id)){
	...
	if(!sgfc_has_editor_permission($entry_id, $current_user->ID)){
		...
		<form method="post">
			<input type="hidden" name="sgfc_business_profile_edit" value="1">
			<input type="hidden" name="sgfc_business_profile_id" value="<?php echo get_the_ID() ?>">
			...
			acf_form(array(
				'field_groups' => array(SGFC_BUSINESS_ACF_ID), // the ID of the field group
				'post_id' => $entry_id,
				'form' => false
			));
